...
public class JogoXadrez {
    ...
    private List<Peca> pecasNaPlaca = new ArrayList<>();
    ...
    public PecaXadrez getEnPassandoVulneravel() {
    ...
    //metodo para mostrar os movimentos possiveis
    ...
    private Peca makeMove(Posicao source, Posicao target) {
        ...
        if(pecaCapturada != null){
            pecasNaPlaca.remove(pecaCapturada);
    ...
    private void undoMove(Posicao source,Posicao target, Peca pecaCapturada){
        ...
        if(pecaCapturada != null){
            ...
            pecasNaPlaca.add(pecaCapturada);
        }
    ...
    private PecaXadrez rei(Color color){
        List<Peca> list = pecasNaPlaca.stream().filter(x -> ((PecaXadrez)x).getColor() == color).collect(Collectors.toList());
        for(Peca p : list){
            if(p instanceof Rei){
                return (PecaXadrez)p;
            }
    ...
    private boolean testCheck(Color color){
        ...
        List<Peca> opponentPecas = pecasNaPlaca.stream().filter(x -> ((PecaXadrez)x).getColor() == opponent(color)).collect(Collectors.toList());
        for(Peca p : opponentPecas){
            boolean[][] matriz = p.movimentoPossiveis();
    ...
    // metodo verificar o xeque do teste matrize
